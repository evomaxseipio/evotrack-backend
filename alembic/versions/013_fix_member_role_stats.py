"""fix member role in stats (member instead of employee)

Revision ID: 013_fix_member_role
Revises: 012_fix_status_case
...
revision = '013_fix_member_role'
down_revision = '012_fix_status_case'
branch_labels = None
depends_on = None


def upgrade() -> None:
    # Drop and recreate function with member role support
        ...
        RETURN (
            ...
            filtered_users AS (
                ...
                WHERE uo.organization_id = p_org_id
                  ...
                  AND (p_status_filter IS NULL OR LOWER(u.status::TEXT) = ANY(p_status_filter))
            ...
            stats_data AS (
                SELECT
                    ...
                    COUNT(*) FILTER (WHERE LOWER(status::TEXT) = 'active') as active_users,
                    COUNT(*) FILTER (WHERE LOWER(status::TEXT) = 'pending_activation') as pending_activation,
                    COUNT(*) FILTER (WHERE LOWER(status::TEXT) = 'inactive') as inactive_users,
                    
                    COUNT(*) FILTER (WHERE role = 'owner') as owners,
                    COUNT(*) FILTER (WHERE role = 'admin') as admins,
                    COUNT(*) FILTER (WHERE role = 'manager') as managers,
                    -- Count both 'employee' and 'member' as employee for backward compatibility
                    COUNT(*) FILTER (WHERE role IN ('employee', 'member')) as employees,
                    
                    COUNT(*) FILTER (WHERE LOWER(status::TEXT) = 'active') as status_active,
                    COUNT(*) FILTER (WHERE LOWER(status::TEXT) = 'pending_activation') as status_pending,
                    COUNT(*) FILTER (WHERE LOWER(status::TEXT) = 'inactive') as status_inactive
                FROM filtered_users
            ),
