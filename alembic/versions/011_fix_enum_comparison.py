"""fix enum comparison in fn_get_organization_users_json

Revision ID: 011_fix_enum_comparison
Revises: 010_update_fn_org_users
...
revision = '011_fix_enum_comparison'
down_revision = '010_update_fn_org_users'
branch_labels = None
depends_on = None


def upgrade() -> None:
    # Drop and recreate function with fixed enum comparisons
        ...
        RETURN (
            ...
            filtered_users AS (
                ...
                WHERE uo.organization_id = p_org_id
                  ...
                  AND (p_status_filter IS NULL OR u.status::TEXT = ANY(p_status_filter))
            ...
            stats_data AS (
                SELECT
                    ...
                    COUNT(*) FILTER (WHERE status::TEXT = 'active') as active_users,
                    COUNT(*) FILTER (WHERE status::TEXT = 'pending_activation') as pending_activation,
                    COUNT(*) FILTER (WHERE status::TEXT = 'inactive') as inactive_users,
                    ...
                    COUNT(*) FILTER (WHERE status::TEXT = 'active') as status_active,
                    COUNT(*) FILTER (WHERE status::TEXT = 'pending_activation') as status_pending,
                    COUNT(*) FILTER (WHERE status::TEXT = 'inactive') as status_inactive
                FROM filtered_users
            ),
...
def downgrade() -> None:
    # Revert to previous version (from migration 010)
    op.execute("DROP FUNCTION IF EXISTS fn_get_organization_users_json(UUID, UUID, INTEGER, JSONB, BOOLEAN, TEXT, TEXT[], TEXT[], BOOLEAN, TIMESTAMPTZ, TIMESTAMPTZ)")
    
    # Recreate the buggy version to allow downgrade
    op.execute("""
    CREATE OR REPLACE FUNCTION fn_get_organization_users_json(
        p_org_id UUID,
        p_current_user_id UUID,
    ...
    DECLARE
        ...
        v_cursor_id UUID;
    BEGIN
        RETURN jsonb_build_object('success', false, 'error', 'Function downgraded - please upgrade');
    END;
    $$;
    """)
